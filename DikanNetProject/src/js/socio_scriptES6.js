  ...
  $('.box-open').each(function () {
    ...
    if ($this.css('display') != 'none') {
      $this.find(':input').each(function () {
        if (!$(this).hasClass("chosen-search-input") && $(this).attr('type') != 'hidden') $(this).addClass('must');
      });
    }
  });
...
$('#otherMili').on('change', function () {
  ...
  if (!e.val().includes('אחר ')) e.val('אחר - ' + e.val());
  $input.val(e.val());
});
$('#addCar').click(function () {
  $.get(carviewurl, function (response) {
    carsdiv.last().before(response); // Reparse the validator for client side validation
...
$('#addFamMem').click(function () {
  $.get(fammemviewurl, function (response) {
    fammemdiv.last().before(response); // Reparse the validator for client side validation
    //form.data('validator', null);
    //$.validator.unobtrusive.parse(form);
  });
});
...
function workSet($this) {
  ...
  var value = $this.val(); //console.log("This: " + $this + "Value : " + value);
  ...
  switch (value) {
    ...
    case 'שכיר':
      $children.each(function () {
        ...
        $(this).find(':input').each(function () {
          if (!$(this).hasClass("chosen-search-input")) $(this).addClass('must');
        });
